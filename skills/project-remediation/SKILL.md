...
- **Verify before acting, verify after.** A review can be stale by the time it is implemented: some items are already done. Before dispatching any item, confirm from the codebase and git history that it is genuinely still open. After a subagent reports success, confirm the intended end state actually holds — never mark an item resolved on a subagent's word alone.
...
- **Anywhere.** Do not commit or push unless the user has asked; leave changes in the working tree with a suggested Conventional Commit message per unit (see `references/subagent-dispatch.md`). Ask before anything that installs heavyweight toolchains, needs credentials, or mutates external state.
...
Before implementing anything, for **each** unit, check whether it is already done — reviews go stale, and a resumed run may re-encounter work a previous session finished. Compare the unit's intended end state (its acceptance criteria) against the current codebase and search the git log for commits implementing it. If the end state already holds, mark the unit `already-resolved`, record the evidence (the commit, the file state), and do not launch a subagent for it. Otherwise mark it `pending`. Do this reconciliation as a distinct pass so that dependency ordering and quota planning in Step 3 see an accurate picture.
...
3. **Dispatch with the reviewer's prompt.** Hand the subagent the recommendation's improvement prompt from `04` verbatim (or, if none exists, a self-contained prompt built from `03`), wrapped in a short orchestration header that overrides its commit/deliverable instructions to match this run's commit policy.
...
5. **Record and clear.** Write to the log what changed, the evidence, the tier used, the verification result, and a suggested Conventional Commit message. Mark **every** ID the unit covers resolved: update the recommendation's status and, for a tech-debt ID, remove or mark its register entry per that register's convention (and remove any in-code references the entry names).
...
- Offer, but do not launch into, next steps: committing the working-tree changes, re-running `project-review` to confirm the fixes, or tackling any deferred/blocked units.
